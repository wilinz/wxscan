...
Map<String, String> readLock(Directory packageRoot) {
  ...
  if (!file.existsSync()) {
    throw StateError('wxscan_core: missing ${file.path}');
  }
...
Future<TfliteLibrary> fetchTflite({
  ...
  String need(String key) =>
      lock[key] ?? (throw StateError('wxscan_core: $key missing from tflite.lock'));
  ...
  if (os == OS.android) {
    final abi = switch (architecture) {
      ...
      // 32-bit emulators.
      _ => throw StateError('wxscan_core: no LiteRT build for $architecture'),
    };
  ...
  if (os == OS.iOS) {
    ...
    if (!out.existsSync()) {
      ...
      if (r.exitCode != 0) {
        throw StateError('wxscan_core: libtool failed: ${r.stderr}');
      }
    }
  ...
  final (slug, ext, libName) = switch ((os, architecture)) {
    ...
    _ => throw StateError('wxscan_core: no TFLite build for $os/$architecture'),
  };
  ...
  if (os == OS.macOS) {
    ...
    if (r.exitCode != 0) {
      throw StateError('wxscan_core: install_name_tool failed: ${r.stderr}');
    }
  }
...
Future<File> _download(String url, File out, String wantSha256) async {
  ...
  try {
    ...
    if (response.statusCode != 200) {
      throw StateError('wxscan_core: GET $url returned ${response.statusCode}');
    }
    ...
    if (got != wantSha256) {
      part.deleteSync();
      throw StateError(
        'wxscan_core: checksum mismatch for $url\n'
        '  expected $wantSha256\n'
        '  got      $got',
      );
    }
...
void _extractOne(File archive, File out, bool Function(String name) matches) {
  ...
  if (found == null) {
    throw StateError('wxscan_core: nothing matched inside ${archive.path}');
  }
  out.parent.createSync(recursive: true);

    ...
    private func handleInitialize(
                ...
                DispatchQueue.main.async {
                    if granted {
                        self.handleInitialize(
                            detect: detect, sr: sr,
                            detectPath: detectPath, srPath: srPath,
                            borrowed: borrowed, result: result)
                    } else {
                        result(FlutterError(
                code: "NO_PERMISSION",
        ...
        sessionQueue.async {
            ...
            DispatchQueue.main.async {
                ...
                // Minted on the main thread, before the map that carries it
                // goes back: what Dart is handed and what the next call reads
